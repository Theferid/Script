...
$method_type = $_SERVER['REQUEST_METHOD'];
    //$_SESSION['demo_error'] = false;
    
if($method_type == "POST" && $_REQUEST['test']){
    
    if(!strpos($_SERVER['REQUEST_URI'],'admin/login')){
         $_SESSION['demo_error'] = true; 
        header("Refresh:0");
         exit;
    }
...
//|| route(0) == 'services'
if ($settings['site_maintenance'] == 2 && $settings["cache"] == 2 && (route(0) == 'faq' || route(0) == 'terms'  || route(0) == 'blog'))
{
